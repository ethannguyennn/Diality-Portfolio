...
def _f_status_pct(col_letter, r, is_done=False):
    ...
    return (f"=LET(\nDoneCount,{count_expr},\nTotalTasks,$G{r},\n"
            f"IF(TotalTasks=0,\"0 (0.0%)\",DoneCount&\" (\"&"
...
def _f_type_pct(col_letter, r):
    ...
    return (f"=LET(\nDoneCount,COUNTIF(INDEX('{SHEET_NAME}'!$E:$E,$B{r}):"
            ...
            f"Denom,$E{r}+$F{r},\n"
            f"IF(Denom=0,\"0 (0.0%)\",DoneCount&\" (\"&"
            f"TEXT(DoneCount/Denom,\"0.0%\")&\")\")\n)")


def _f_person_pct(col_letter, r):
    # "<done> / <assigned> (<pct>)" for one team member's column, keyed on
    # the column's own header. Exception: an unassigned person shows a bare
    # "0 / 0" with the percentage omitted, since a 0/0 rate has no
